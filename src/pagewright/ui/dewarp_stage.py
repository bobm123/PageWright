...
# (re-exported for back-compat; new code should import from .display)
...
class DewarpStageWidget(QWidget):
    ...
    def _show_source_menu(self, info):
        ...
        m_out = menu.addMenu("Outline")
        sv = m_out.addAction("Save Outline...", self._save_outline)
        sv.setEnabled(self._spline_mode()
                      and self._source.model() is not None)
        m_out.addAction("Load Outline...", self._load_outline)
        m_img = menu.addMenu("Image")
        m_img.addAction("Rotate 90 deg CW",
                        lambda: self._rotate_source(True))
        m_img.addAction("Rotate 90 deg CCW",
                        lambda: self._rotate_source(False))
        m_img.addAction("Flip Horizontal",
                        lambda: self._flip_source(True))
        m_img.addAction("Flip Vertical",
                        lambda: self._flip_source(False))
        menu.addSeparator()
